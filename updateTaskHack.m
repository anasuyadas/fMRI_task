...
if stimulus.FixationBreak(stimulus.trialnum)
    n = length(task.fixBreakTrial);
    task.fixBreakTrial{n+1} = task.thistrial;
    stimulus.numTrials = stimulus.numTrials + 1; 
end

%only update randvars if this is a new trial and not a trial that is been
if stimulus.trialnum <= task.origNumTrials
