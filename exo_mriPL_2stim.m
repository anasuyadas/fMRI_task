function myscreen = exo_mri_pilot_2stim(observer,varargin)
...
if ieNotDefined('indContrast'),indContrast = [0 .1 .8];end % initialize some default contrast vals
...
task{1}.segmax =     [0.1 .06 .04 .1 .3 .3 .64 .03 1];  % ITI is variable 
...
if diagonal 
    [contrast, iti, location,repeats] = ndgrid(1:3,1:3,1:2,1:n_repeats);
else 
    [contrast, iti, location,repeats] = ndgrid(1:3,1:3,3:4,1:n_repeats);
end
...
myscreen = initStimulus('stimulus',myscreen,indContrast,diagonal);
